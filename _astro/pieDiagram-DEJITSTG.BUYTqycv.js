...
`,"getStyles"),bt=Mt,kt=d(t=>{const a=[...t.values()].reduce((s,p)=>s+p,0),f=[...t.entries()].map(([s,p])=>({label:s,value:p})).filter(s=>s.value/a*100>=1);return mt().value(s=>s.value).sort(null)(f)},"createPieArcs"),Et=d((t,a,f,y)=>{F.debug(`rendering pie chart
`+t);const s=y.db,p=rt(),o=it(s.getConfig(),p.pie),e=40,r=18,l=4,g=450,h=g,v=st(a),c=v.append("g");c.attr("transform","translate("+h/2+","+g/2+")");const{themeVariables:i}=p;let[x]=ot(i.pieOuterStrokeWidth);x??=2;const w=o.textPosition,m=Math.min(h,g)/2-e,D=P().innerRadius(0).outerRadius(m),$=P().innerRadius(m*w).outerRadius(m*w);c.append("circle").attr("cx",0).attr("cy",0).attr("r",m+x/2).attr("class","pieOuterCircle");const u=s.getSections(),A=kt(u),C=[i.pie1,i.pie2,i.pie3,i.pie4,i.pie5,i.pie6,i.pie7,i.pie8,i.pie9,i.pie10,i.pie11,i.pie12];let M=0;u.forEach(n=>{M+=n});const B=A.filter(n=>(n.data.value/M*100).toFixed(0)!=="0"),b=gt(C).domain([...u.keys()]);c.selectAll("mySlices").data(B).enter().append("path").attr("d",D).attr("fill",n=>b(n.data.label)).attr("class","pieCircle"),c.selectAll("mySlices").data(B).enter().append("text").text(n=>(n.data.value/M*100).toFixed(0)+"%").attr("transform",n=>"translate("+$.centroid(n)+")").style("text-anchor","middle").attr("class","slice");const V=c.append("text").text(s.getDiagramTitle()).attr("x",0).attr("y",-400/2).attr("class","pieTitleText"),G=[...u.entries()].map(([n,E])=>({label:n,value:E})),k=c.selectAll(".legend").data(G).enter().append("g").attr("class","legend").attr("transform",(n,E)=>{const O=r+l,Q=O*G.length/2,X=12*r,q=E*O-Q;return"translate("+X+","+q+")"});k.append("rect").attr("width",r).attr("height",r).style("fill",n=>b(n.label)).style("stroke",n=>b(n.label)),k.append("text").attr("x",r+l).attr("y",r-l).text(n=>s.getShowData()?`${n.label} [${n.value}]`:n.label);const U=Math.max(...k.selectAll("text").nodes().map(n=>n?.getBoundingClientRect().width??0)),Z=h+e+r+l+U,L=V.node()?.getBoundingClientRect().width??0,j=h/2-L/2,K=h/2+L/2,N=Math.min(0,j),I=Math.max(Z,K)-N;v.attr("viewBox",`${N} 0 ${I} ${g}`),lt(v,g,I,o.useMaxWidth)},"draw"),Rt={draw:Et},Nt={parser:Tt,db:_,renderer:Rt,styles:bt};export{Nt as diagram};
